...
public final class CompiledFlowRuntime {
    ...
    public void trigger(String inputNodeId, RuntimeMessage message) {
        ...
        if (input.category() != NodeCategory.INPUT) throw new IllegalArgumentException("Node " + inputNodeId + " is not an INPUT node");
    ...
    private void dispatch(String sourceNodeId, String sourcePort, RuntimeMessage message) {
        ...
        for (String targetNodeId : targets) {
            if (!flow.connectionEnabled(sourceNodeId, sourcePort, targetNodeId)) continue;
    ...
    private void execute(CompiledNode node, RuntimeMessage message) {
        switch (node.category()) {
            ...
            case OUTPUT -> {
                ...
                if (handler == null) throw new IllegalStateException("No output handler registered for node " + node.id());
    ...
    private void executeCompiled(CompiledNode node, RuntimeMessage message) {
        ...
        if (executable == null) throw new IllegalStateException("Executor node " + node.id() + " has no compiled executable");
    ...
    private final class RuntimeExecutionContext implements NexaExecutionContext {
        ...
        private RuntimeExecutionContext(String sourceNodeId) { this.sourceNodeId = sourceNodeId; }
        ...
        @Override public void send(List<String> ports, RuntimeMessage msg) {
            requireMessage(msg);
            for (String port : new ArrayList<>(ports)) send(port, msg);
        }

        @Override public Object callHostCapability(String namespace, String name, List<Object> args) {
            throw new UnsupportedOperationException("Host capability is not available in the minimal runtime: " + namespace + "." + name);
        }

        @Override public int readTagInt(int slot) { return tags().readInt(slot); }
