...
public final class FastTagStore {
    ...
    public FastTagStore(List<TagDefinition> definitions) {
        ...
        types = new ArrayList<>(definitions.size());
        paths = new ArrayList<>(definitions.size());
        int ints=0, longs=0, doubles=0, objects=0, slot=0;
        for (TagDefinition definition : definitions) {
            if (definition.name()==null || definition.name().isBlank()) throw new IllegalArgumentException("Tag name must not be blank");
            if (slots.putIfAbsent(definition.name(), slot++) != null) throw new IllegalArgumentException("Duplicate tag: "+definition.name());
            paths.add(definition.name()); types.add(definition.dataType());
        ...
        intValues=new int[ints]; longValues=new long[longs]; doubleValues=new double[doubles]; objectValues=new Object[objects];
    ...
    private void initialize(List<TagDefinition> definitions) {
        ...
        for(int slot=0;slot<types.size();slot++) switch(types.get(slot)) {
        ...
        for(int slot=0;slot<definitions.size();slot++) if(definitions.get(slot).value()!=null) writeSlot(slot, definitions.get(slot).value(), TagQuality.GOOD, false);
    }
    ...
    public Object readSlot(int slot) { return switch(types.get(slot)) {
    ...
    public TagValue write(String path,Object value,TagQuality quality){return writeSlot(slot(path),value,quality,true);}
    public TagValue writeSlot(int slot,Object value,TagQuality quality,boolean notify){
        Object old=readSlot(slot); store(slot,value); TagValue event=notify&&(!writeListeners.isEmpty()||!changeListeners.isEmpty())?new TagValue(paths.get(slot),value==null?null:old,old,value,System.currentTimeMillis(),quality):null;
        if(event!=null){for(var listener:writeListeners)listener.accept(event);if(!Objects.equals(old,value))for(var listener:changeListeners)listener.accept(event);} return event;
    }
    private void store(int slot,Object value){switch(types.get(slot)){
