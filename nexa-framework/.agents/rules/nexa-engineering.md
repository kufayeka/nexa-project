...
Penerapan Dependency Inversion Principle (DIP) wajib diimplementasikan menggunakan antarmuka (interface) publik di paket `api/` untuk komunikasi antar-domain.
...
Keep dependencies pointing toward the business core and through interfaces.
...
Setiap paket wajib mempresentasikan satu Domain fungsional (Domain-Driven Packaging) dengan pembagian sub-paket standar (api, controller, service, model, registry, helpers, internal).
...
Pajang hanya antarmuka di dalam sub-paket `api/`. Sembunyikan seluruh kelas implementasi di dalam sub-paket `service/` dengan menyetel visibilitas kelas menjadi *package-private*.
