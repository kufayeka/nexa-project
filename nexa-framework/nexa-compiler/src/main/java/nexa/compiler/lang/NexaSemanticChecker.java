...
/**
 * Final Workstream-1 semantic checker. OBJECT is the dynamic object supertype;
 * all primitive conversions remain strict.
 */
public final class NexaSemanticChecker {
    ...
    public NexaSemanticChecker() {
        for (NexaType t : List.of(NexaType.BOOLEAN, NexaType.INT8, NexaType.INT16, NexaType.INT32, NexaType.INT64,
                NexaType.UINT8, NexaType.UINT16, NexaType.UINT32, NexaType.UINT64, NexaType.FLOAT32, NexaType.FLOAT64,
                NexaType.STRING, NexaType.OBJECT, NexaType.VOID))
    ...
    public List<Diagnostic> check(Program p) {
        errors.clear();
        scopes.clear();
        scopes.push(new HashMap<>(Map.of("self", NexaType.OBJECT, "input", NexaType.OBJECT)));
    ...
    private void stmt(Stmt s) {
        if (s instanceof TypeDecl d) {
            if (types.containsKey(d.name()))
                bad("Type already defined: " + d.name(), d.span());
            else
                types.put(d.name(), resolve(d.type()));
            return;
        }
        if (s instanceof Let l) {
            NexaType v = expr(l.init());
            require(resolve(l.type()), v, l.init(), "initializer");
            define(l.name(), resolve(l.type()));
            return;
        }
        if (s instanceof Assign a) {
            if (!lvalue(a.target()))
                bad("Assignment target is not writable", a.target().span());
            NexaType t = expr(a.target()), v = expr(a.value());
            require(t, v, a.value(), "assignment");
            return;
        }
        ...
        if (s instanceof For f) {
            NexaType it = expr(f.iterable());
            if (!(it instanceof NexaType.Array a))
                bad("'in' requires ARRAY<T>", f.iterable().span());
            else {
                require(resolve(f.declaredType()), a.element(), f.iterable(), "loop variable");
    ...
    private NexaType expr(Expr e) {
        ...
        if (e instanceof Var v) {
            ...
            if (t == null) {
                bad("Unknown variable: " + v.name(), v.span());
        ...
        if (e instanceof Field f) {
            ...
            if (t instanceof NexaType.ObjectType o) {
                NexaType v = o.fields().get(f.name());
                if (v == null) {
                    bad("Unknown field '" + f.name() + "'", f.span());
                    return NexaType.OBJECT;
                }
                return resolve(v);
            }
            if (NexaType.same(t, NexaType.OBJECT))
                return NexaType.OBJECT;
            bad("Field access requires OBJECT/struct", f.span());
            return NexaType.OBJECT;
        }
        if (e instanceof Index i) {
            ...
            if (t instanceof NexaType.Array a) {
                require(NexaType.INT64, idx, i.index(), "array index");
                return a.element();
            }
            if (NexaType.same(t, NexaType.OBJECT))
                return NexaType.OBJECT;
            bad("Indexing requires ARRAY or OBJECT", i.span());
            return NexaType.OBJECT;
        }
        if (e instanceof Array a) {
            ...
            for (Expr x : a.values()) {
                NexaType xt = expr(x);
                et = et == null ? xt : common(et, xt, x.span());
            }
            return new NexaType.Array(et == null ? NexaType.OBJECT : et);
        }
        if (e instanceof ObjectLit o) {
            Map<String, NexaType> f = new LinkedHashMap<>();
            for (var x : o.fields().entrySet())
                f.put(x.getKey(), expr(x.getValue()));
            return new NexaType.ObjectType(f);
        }
        if (e instanceof Unary u) {
            NexaType t = expr(u.expr());
            if (u.op().equals("!")) {
                require(NexaType.BOOLEAN, t, u.expr(), "logical negation");
                return NexaType.BOOLEAN;
            }
            if (!NexaType.numeric(t))
                bad("Unary numeric operator requires numeric value", u.span());
            return t;
        }
        if (e instanceof Binary b) {
            NexaType l = expr(b.left()), r = expr(b.right());
            if (b.op().equals("&&") || b.op().equals("||")) {
                require(NexaType.BOOLEAN, l, b.left(), "logical operand");
                require(NexaType.BOOLEAN, r, b.right(), "logical operand");
                return NexaType.BOOLEAN;
            }
            if (Set.of("==", "!=", "<", "<=", ">", ">=").contains(b.op())) {
                if (!compatible(l, r))
                    bad("Incompatible comparison types", b.span());
                return NexaType.BOOLEAN;
            }
            if (!NexaType.numeric(l) || !NexaType.numeric(r))
                bad("Arithmetic requires numeric operands", b.span());
    ...
    private NexaType common(NexaType a, NexaType b, SourceSpan s) {
        ...
        if (NexaType.numeric(a) && NexaType.numeric(b)) {
            if (a.displayName().equals("FLOAT64") || b.displayName().equals("FLOAT64"))
                return NexaType.FLOAT64;
            if (a.displayName().equals("FLOAT32") || b.displayName().equals("FLOAT32"))
                return NexaType.FLOAT32;
            return rank(a) >= rank(b) ? a : b;
        }
        bad("No compatible types: " + a.displayName() + " and " + b.displayName(), s);
        return NexaType.OBJECT;
    }

    private boolean compatible(NexaType a, NexaType b) {
        return NexaType.same(resolve(a), resolve(b)) || (NexaType.numeric(a) && NexaType.numeric(b));
    }
    ...
    private void require(NexaType target, NexaType value, Expr source, String where) {
        ...
        // OBJECT accepts structured/dynamic object values.
        if (NexaType.same(target, NexaType.OBJECT)
                && value instanceof NexaType.ObjectType) {
            return;
        }

        // Validate array elements against the declared element type.
        if (target instanceof NexaType.Array ta
                && source instanceof Array array) {

            for (Expr element : array.values()) {
                NexaType elementType = expr(element);
        ...
        // Numeric literals may fit into a narrower declared type.
        if (source instanceof Literal l && literalFits(l, target)) {
            return;
        }
        ...
        if (NexaType.numeric(target)
                ...
                && !(value.displayName().startsWith("UINT")
                        && !target.displayName().startsWith("UINT"))) {
            return;
        }
        ...
        bad(
                "Type mismatch in " + where
                        ...
                        + ", got " + value.displayName(),
                source.span());
    }

    private boolean literalFits(Literal l, NexaType t) {
        if (!(l.value() instanceof Number n) || !NexaType.numeric(t))
            return false;
    ...
    private NexaType resolve(NexaType t) {
        if (t instanceof NexaType.Named n) {
            ...
            if (r == null) {
                bad("Unknown type: " + n.name(), new SourceSpan(0, 0));
                return NexaType.OBJECT;
            }
            return r;
        }
        if (t instanceof NexaType.Array a)
            return new NexaType.Array(resolve(a.element()));
        if (t instanceof NexaType.ObjectType o) {
            Map<String, NexaType> f = new LinkedHashMap<>();
            o.fields().forEach((k, v) -> f.put(k, resolve(v)));
            return new NexaType.ObjectType(f);
        }
        return t;
    }

    private boolean lvalue(Expr e) {
        return e instanceof Var || e instanceof Field || e instanceof Index;
    }

    private void define(String n, NexaType t) {
        scopes.peek().put(n, t);
    }

    private NexaType lookup(String n) {
        for (var s : scopes)
            if (s.containsKey(n))
                return s.get(n);
        return null;
    ...
    private void bad(String m, SourceSpan s) {
        errors.add(new Diagnostic(m, s));
    }
}
