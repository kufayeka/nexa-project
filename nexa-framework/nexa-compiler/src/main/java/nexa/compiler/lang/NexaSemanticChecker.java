...
public final class NexaSemanticChecker {
    ...
    private void stmt(Stmt statement) {
        ...
        if (statement instanceof Assign assignment) {
            if (!lvalue(assignment.target())) bad("Assignment target is not writable", assignment.target().span());
    ...
    private void require(NexaType target, NexaType value, Expr source, String where) {
        ...
        // OBJECT is Nexa's dynamic top type. A value stored into a dynamic
        // object slot is intentionally unchecked at compile time; the runtime
        // boundary is responsible for validating it when a concrete contract
        // exists (plugin/host/object schema).
    ...
    private NexaType lookupSymbol(String name) {
        for (Map<String, NexaType> scope : scopes) if (scope.containsKey(name)) return scope.get(name);
        return null;
    }
    private NexaType lookup(String name) { return lookupSymbol(name); }
