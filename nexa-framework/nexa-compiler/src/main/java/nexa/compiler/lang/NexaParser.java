...
public final class NexaParser {
    ...
    /** Integer literals use the smallest stable signed integer type that can represent them. */
    private Expr integerLiteral(NexaToken token) {
        long value = Long.parseLong(token.text());
        NexaType type = (value >= Integer.MIN_VALUE && value <= Integer.MAX_VALUE)
                ? NexaType.INT32 : NexaType.INT64;
        return new Literal(value, type, token.span());
    }
