...
public final class NexaBytecodeCompiler {
    ...
    private void emit(MethodVisitor mv, NexaIr.Instruction i, Slots s) {
        if (i instanceof NexaIr.Const x) {
            ...
            if (isArray(x.target().type())) {
                mv.visitVarInsn(Opcodes.ALOAD, key); unbox(mv, NexaType.INT32);
            ...
            load(mv, x.iterator(), s);
            mv.visitMethodInsn(Opcodes.INVOKEINTERFACE, ITERATOR, "hasNext", "()Z", true); store(mv, x.result(), s);
        } else if (i instanceof NexaIr.IterNext x) {
            load(mv, x.iterator(), s);
            mv.visitMethodInsn(Opcodes.INVOKEINTERFACE, ITERATOR, "next", "()Ljava/lang/Object;", true);
            unbox(mv, x.result().type()); store(mv, x.result(), s);
        } else if (i instanceof NexaIr.Return x) {
            if (x.value() != null) { load(mv, x.value(), s); mv.visitInsn(Opcodes.POP); }
            mv.visitInsn(Opcodes.RETURN);
    ...
    private void call(MethodVisitor mv, NexaIr.Call x, Slots s) {
        ...
        if (x.args().size() == 1) {
            load(mv, x.args().getFirst(), s); int arg = s.scratch(); mv.visitVarInsn(Opcodes.ASTORE, arg);
            ...
            load(mv, x.args().get(0), s); int a = s.scratch(); mv.visitVarInsn(Opcodes.ASTORE, a);
            load(mv, x.args().get(1), s); int b = s.scratch(); mv.visitVarInsn(Opcodes.ASTORE, b);
            mv.visitVarInsn(Opcodes.ALOAD, 2); mv.visitVarInsn(Opcodes.ALOAD, a); mv.visitVarInsn(Opcodes.ALOAD, b);
            String desc = isString(x.args().get(0).type()) ? "(Ljava/lang/String;L" + MSG + ";)V" : "(Ljava/util/List;L" + MSG + ";)V";
    ...
    private void binary(MethodVisitor mv, NexaIr.Binary x, Slots s) {
        switch (x.op()) {
            case "&&" -> logicalAnd(mv, x, s);
            case "||" -> logicalOr(mv, x, s);
            case "+" -> {
                if (isString(x.result().type()) || isString(x.left().type()) || isString(x.right().type())) concat(mv, x, s);
                else arithmetic(mv, x, s, "+");
            }
    ...
    private void arithmetic(MethodVisitor mv, NexaIr.Binary x, Slots s, String op) {
        load(mv, x.left(), s); convert(mv, x.left().type(), x.result().type());
        load(mv, x.right(), s); convert(mv, x.right().type(), x.result().type());
    ...
    private void compare(MethodVisitor mv, NexaIr.Binary x, Slots s) {
        if (isString(x.left().type()) || isString(x.right().type())) {
            ...
            load(mv, x.left(), s); load(mv, x.right(), s);
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, "java/util/Objects", "equals", "(Ljava/lang/Object;Ljava/lang/Object;)Z", false);
            if ("!=".equals(x.op())) not(mv);
            return;
        }
        NexaType common = x.left().type();
        load(mv, x.left(), s); convert(mv, x.left().type(), common);
        load(mv, x.right(), s); convert(mv, x.right().type(), common);
        if (isLong(common)) mv.visitInsn(Opcodes.LCMP);
        else if (isDouble(common)) mv.visitInsn(Opcodes.DCMPL);
        else if (isFloat(common)) mv.visitInsn(Opcodes.FCMPL);
        ...
        if (!isLong(common) && !isDouble(common) && !isFloat(common)) {
            // For int-like values the two operands are still on stack; compare directly.
            mv.visitJumpInsn(switch (x.op()) { case "==" -> Opcodes.IF_ICMPEQ; case "!=" -> Opcodes.IF_ICMPNE; case "<" -> Opcodes.IF_ICMPLT; case "<=" -> Opcodes.IF_ICMPLE; case ">" -> Opcodes.IF_ICMPGT; default -> Opcodes.IF_ICMPGE; }, yes);
        } else mv.visitJumpInsn(op, yes);
    ...
    private void logicalAnd(MethodVisitor mv, NexaIr.Binary x, Slots s) {
        Label falseLabel = new Label(), done = new Label(); load(mv, x.left(), s); mv.visitJumpInsn(Opcodes.IFEQ, falseLabel); load(mv, x.right(), s); mv.visitJumpInsn(Opcodes.IFEQ, falseLabel); mv.visitInsn(Opcodes.ICONST_1); mv.visitJumpInsn(Opcodes.GOTO, done); mv.visitLabel(falseLabel); mv.visitInsn(Opcodes.ICONST_0); mv.visitLabel(done);
    }

    private void logicalOr(MethodVisitor mv, NexaIr.Binary x, Slots s) {
        Label trueLabel = new Label(), done = new Label(); load(mv, x.left(), s); mv.visitJumpInsn(Opcodes.IFNE, trueLabel); load(mv, x.right(), s); mv.visitJumpInsn(Opcodes.IFNE, trueLabel); mv.visitInsn(Opcodes.ICONST_0); mv.visitJumpInsn(Opcodes.GOTO, done); mv.visitLabel(trueLabel); mv.visitInsn(Opcodes.ICONST_1); mv.visitLabel(done);
    }
    ...
    private void append(MethodVisitor mv, NexaIr.Value v, Slots s) {
        load(mv, v, s);
        String d = switch (v.type().displayName()) { case "BOOLEAN" -> "(Z)"; case "INT8", "UINT8" -> "(I)"; case "INT16", "UINT16" -> "(I)"; case "INT32", "UINT32" -> "(I)"; case "INT64", "UINT64" -> "(J)"; case "FLOAT32" -> "(F)"; case "FLOAT64" -> "(D)"; case "STRING" -> "(Ljava/lang/String;)"; default -> "(Ljava/lang/Object;)"; };
        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/lang/StringBuilder", "append", d + "Ljava/lang/StringBuilder;", false);
    }

    private void constant(MethodVisitor mv, Object value, NexaType type) {
        ...
        else if (value instanceof Number n && !isLong(type) && !isFloat(type) && !isDouble(type)) mv.visitLdcInsn(n.intValue());
    ...
    private void convert(MethodVisitor mv, NexaType from, NexaType to) {
        if (!NexaType.numeric(from) || !NexaType.numeric(to) || from.displayName().equals(to.displayName())) return;
        if (isDouble(to)) { if (isLong(from)) mv.visitInsn(Opcodes.L2D); else if (isFloat(from)) mv.visitInsn(Opcodes.F2D); else mv.visitInsn(Opcodes.I2D); }
        else if (isFloat(to)) { if (isLong(from)) mv.visitInsn(Opcodes.L2F); else mv.visitInsn(Opcodes.I2F); }
        else if (isLong(to) && !isLong(from)) mv.visitInsn(Opcodes.I2L);
        else if (!isLong(to) && isLong(from)) mv.visitInsn(Opcodes.L2I);
    }
    ...
    private boolean isReference(NexaType t) { return !NexaType.numeric(t) && !isBoolean(t); }
    private boolean isBoolean(NexaType t) { return "BOOLEAN".equals(t.displayName()); }
    private boolean isLong(NexaType t) { return "INT64".equals(t.displayName()) || "UINT64".equals(t.displayName()); }
    private boolean isFloat(NexaType t) { return "FLOAT32".equals(t.displayName()); }
    ...
    private String className(String program, String function) {
        ...
        for (int i = 0; i < raw.length(); i++) out.append(Character.isJavaIdentifierPart(raw.charAt(i)) || raw.charAt(i) == '/' ? raw.charAt(i) : '_');
        return out.toString();
    }
    ...
    private static final class Slots {
        private final Map<Integer,Integer> values; private final Map<String,Integer> locals; private final Map<String,NexaType> types; private int nextScratch;
        private Slots(Map<Integer,Integer> values, Map<String,Integer> locals, Map<String,NexaType> types, int nextScratch) { this.values=values; this.locals=locals; this.types=types; this.nextScratch=nextScratch; }
        static Slots build(NexaIr.Function f) {
            Map<Integer,NexaType> resultTypes = new TreeMap<>();
            for (NexaIr.Block b : f.blocks()) for (NexaIr.Instruction i : b.instructions()) if (i.result()!=null) resultTypes.put(i.result().id(), i.result().type());
            Map<Integer,Integer> values = new HashMap<>(); int slot=5;
            for (var e: resultTypes.entrySet()) { values.put(e.getKey(),slot); slot += wide(e.getValue())?2:1; }
            Map<String,Integer> locals=new LinkedHashMap<>(); Map<String,NexaType> types=new LinkedHashMap<>();
            for (NexaIr.Local l:f.locals()) if (!locals.containsKey(l.name())) { locals.put(l.name(),slot); types.put(l.name(),l.type()); slot += wide(l.type())?2:1; }
            return new Slots(values,locals,types,slot);
        }
